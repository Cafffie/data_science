...
iris[0:5, c("Sepal.Length","Species")]
iris["Sepal.Length"]
...
sum(is.na(germany3$Solar))
sum(is.na(germany3$Wind))
sum(is.na(germany3$Wind.Solar))
...
year <- as.numeric(format(column, "%Y"))
...
freq_subset  %>%
  ggplot(aes(tokens, n, fill=tokens)) +
...
#Creating a visual representation with wordcloud
set.seed(234)
